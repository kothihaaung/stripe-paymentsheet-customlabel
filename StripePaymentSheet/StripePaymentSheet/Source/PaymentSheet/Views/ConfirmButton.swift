            ...
            let text: String? = {
                ...
                case .enabled, .disabled, .spinnerWithInteractionDisabled:
                    ...
                    case let .pay(amount, currency):
                        if let payText = PaymentSheet.Configuration.customLables["Pay"] {
                            let localizedAmount = String.localizedAmountDisplayString(
                                for: amount, currency: currency)
                            return String(format: "%@ %@", payText, localizedAmount)
                        } else {
                            let localizedAmount = String.localizedAmountDisplayString(
                                for: amount, currency: currency)
                    ...
                    case .setup:
                        if let setupText = PaymentSheet.Configuration.customLables["Set up"] {
                            return setupText
